<template>
    <el-card style="border-radius: 15px;width: 100%;">
        ...
        <el-form :model="form" size="large" label-position="top">
            ...
            <el-form-item class="question" v-if="showPCQ01" style="font-weight: bolder;"
                label="C0102.专利权人所在机构/企业是否生产该产品？">
            ...
            <el-form-item class="question" v-if="showPCQ01" style="font-weight: bolder;"
                label="C0105.如何形容专利权人所在机构/企业相关产品的市场份额？">
            ...
            <el-form-item class="question" v-if="form.pCq01" style="font-weight: bolder;"
                label="C0106.作为流程创新，该专利技术能降低多少生产成本？">
                <el-radio-group v-model="form.pCq0106">
            ...
            <el-form-item class="question" style="font-weight: bolder;" label="C02.如何形容该专利技术的研发成本？请估计下已经耗费的人工年？">
            ...
            <el-form-item class="question" style="font-weight: bolder;" label="C03.该专利技术是否有研发合作？如果有，是与哪类机构/企业进行合作？">
                <el-checkbox-group v-model="form.pCq03" style="display: flex;flex-wrap: wrap;"
                    @change="handlePCQ03Change">
            ...
            <el-form-item class="question" style="font-weight: bolder;" label="C04.该专利技术的研发经费来自以下哪些渠道？">
            ...
            <el-form-item class="question" style="font-weight: bolder;" label="C05.您认为该技术是否解决了行业内的技术瓶颈？">
                <el-radio-group v-model="form.pCq05">
                    <el-radio class="answer" label="是" />
            ...
            <el-form-item class="question" style="font-weight: bolder;" label="C06.该专利技术是否遭受过侵权？">
            ...
            <el-form-item class="question" style="font-weight: bolder;" label="C07.如何评价其他知识产权保护对该专利技术产品的重要性？">
                <el-form-item class="question blue-label" style="font-weight: bolder;" label="（请打分，1🌟为不重要，5🌟为非常重要）" />
                <el-table :data="tablePCQ7" style="width: 100%" :row-style="{ height: '10px' }"
                    :cell-style="{ padding: '0px' }">
                    <el-table-column class="answer" width="250%">
                        <template #default="{ row, $index }">
                            <template v-if="$index !== tablePCQ7.length - 1">
                                {{ row.name }}
                            </template>
                            <template v-else>
                                <el-input v-model="tablePCQ7[$index].name" style="height: 3vh;"
                                    placeholder="其他，请说明"></el-input>
                            </template>
                        </template>
                    </el-table-column>
                    <el-table-column width="250%" class="answer" header-align="center"
                        v-for="(column, colIndex) in colPCQ7" :key="colIndex" :label="column.label">
                        <template #default="{ row }">
                            <el-rate text-color="#ff9900" size="large" v-model="row.rate"
                                :texts="['not', 'so-so', 'relative', 'very', 'super']" show-text class="table-container"
                                @change="handlePCQ7(row, colIndex)" />
                        </template>
                    </el-table-column>
                </el-table>
            </el-form-item>

            <el-form-item class="question" style="font-weight: bolder;" label="C08.在未来三年内，专利权人所在公司/机构是否会继续开发该专利技术？">
                <el-radio-group v-model="form.pCq08">
            ...
            <el-form-item class="question" style="font-weight: bolder;" label="C09.如果减少或者不再开发，以下原因的影响有多大？">
                <el-form-item class="question blue-label" style="font-weight: bolder;" label="（请打分，1🌟为不影响，5🌟为非常影响）" />
                <el-table :data="tablePCQ09" style="width: 100%" :row-style="{ height: '10px' }"
                    :cell-style="{ padding: '0px' }">
                    <el-table-column class="answer" width="250%">
                        <template #default="{ row }">
                            {{ row.name }}
                    ...
                    <el-table-column width="250%" class="answer" header-align="center"
                        v-for="(column, colIndex) in colPCQ09" :key="colIndex" :label="column.label">
                        <template #default="{ row }">
                            <el-rate text-color="#ff9900" size="large" v-model="row.rate"
                                :texts="['not', 'so-so', 'relative', 'very', 'super']" show-text class="table-container"
                                @change="handlePCQ09(row, colIndex)" />
                        </template>
                    </el-table-column>
                </el-table>
            </el-form-item>

            <el-form-item class="question" style="font-weight: bolder;" label='C10.该专利是否属于专利组合的一部分？'>
                <el-radio-group v-model="form.pCq10">
            ...
            <el-form-item class="question" style="font-weight: bolder;" label="C11.假设有第三方购买该专利（专利组合），您预计最低价格是多少？">
...
const handlePCQ03Change = (value) => {
    if (value.includes('没有，完全由本单位研发')) {  
        form.pCq03 = ['没有，完全由本单位研发']; 
    }
};
...
// 处理单元格选中状态变化
const handlePCQ7 = (row, colIndex) => {
    // 取消当前行其他单元格的选中状态
    form.pCq07 = tablePCQ7
};

const tablePCQ09 = ref([
...
// 处理单元格选中状态变化
const handlePCQ09 = (row, colIndex) => {
    // 取消当前行其他单元格的选中状态
    form.pCq09 = tablePCQ09
};
